...
const main = async (msg, params)=>{
    ...
    const tierEmoji = client.emojis.find(emoji => emoji.name === `ruby1`)

    const embed = new RichEmbed()
    .setColor(solved.color[Math.floor((targetInfo.level-1)/5)])
    .setTitle(`**${targetInfo.user}**`)
    .setDescription(`**${targetInfo.tier}**${tierEmoji}`)
    .attachFile(`packages/백준/res/${targetInfo.class}.png`)
    .setThumbnail(`attachment://${targetInfo.class}.png`)
    .addBlankField()
    .addField(`**총 경험치**`, `${targetInfo.xp}`)
    .addField(`**푼 문제**`, `${targetInfo.submission}`, true)
    .addField(`**정답**`, `${targetInfo.solved}`, true)
...
module.exports = {
    func : main,
    keyword : 'user',
    help : '백준 체크 리스트에 등록된 사용자 한명을 확인합니다.',
    args : 'id'
}