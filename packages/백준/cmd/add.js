module.exports = {
    func: (msg, params) => {
        ...
        if (!guild.hasOwnProperty('target')) {
            // init이 안된 경우
            msg.channel.send(
                `${bot.setting[gid].prefix}bjinit을 통해 학교 등록 후 사용해주세요. 자세한 것은 도움말을 참고해주세요.`
            )
            ...
            if (guild.users.includes(params[0])) {
                ...
                msg.channel.send(
                    `백준 아이디 ${params[0]}는 이제 ${
                        msg.guild.members.get(params[1]).displayName
                    }님의 아이디입니다.`
                )
            } else {
                // 신규 사용자 (id 지정)
                ...
                guild.uid[params[0]] = params[1]
                msg.channel.send(
                    `${msg.guild.members.get(params[1]).displayName}님의 백준 아이디 ${
                        params[0]
                    }이 등록되었습니다.`
                )
            }
            ...
            if (guild.users.includes(params[0])) {
                ...
                msg.channel.send(
                    `${msg.member.displayName}님의 백준 아이디 ${params[0]}이 등록되었습니다.`
                )
            }
            store.save(`백준/${gid}`)
        } else {
