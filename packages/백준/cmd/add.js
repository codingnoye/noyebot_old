    ...
    func : (msg, params)=>{
        ...
        if (!guild.hasOwnProperty('target')) {
            ...
            if (guild.users.includes(params[0])) {
                ...
                msg.channel.send(`백준 아이디 ${params[0]}는 이제 ${msg.guild.members.get(params[1]).displayName}님의 아이디입니다.`)
            } else {
                // 신규 사용자 (id 지정)
                guild.users.push(params[0])
                guild.level[params[0]] = null
                guild.uid[params[0]] = params[1]
                msg.channel.send(`${msg.guild.members.get(params[1]).displayName}님의 백준 아이디 ${params[0]}이 등록되었습니다.`)
            }
            store.save(`백준/${gid}`)
            ...
            if (guild.users.includes(params[0])) {
                ...
                msg.channel.send(`${msg.member.displayName}님의 백준 아이디 ${params[0]}이 등록되었습니다.`)
            }
            store.save(`백준/${gid}`)
        } else {
