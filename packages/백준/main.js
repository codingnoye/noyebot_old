...
const package = {
    ...
    onCmd(msg, params) {
        if (params[0] == 'bj') return commands.call(msg, params[1], params.slice(2))
        else return false
    },
    ...
    help(msg) {
        ...
        for (const keyword of commands.keywords) {
            ...
            if (help.args)
                embed.addField(
                    `\`${prefix}bj ${keyword} ${help.args
                        .split(' ')
                        .map((x) => '<' + x + '>')
                        .join(' ')}\``,
                    help.desc
                )
            else embed.addField(`\`${prefix}bj ${keyword}\``, help.desc)
        }
    ...
    helpSimple(msg, embed) {
        ...
        for (const keyword of commands.keywords) {
            ...
            else helps.push(`\`${prefix}bj ${keyword}\`\n${help.desc}`)
        }
