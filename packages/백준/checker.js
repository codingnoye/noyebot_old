            ...
            for (const solution of solutions.filter((solution) => solution.id > guild.lastcheck)) {
                ...
                if (guild.users.includes(solution.user)) {
                    ...
                    const embed = new RichEmbed()
                        .setTitle(
                            `**${user}**(${
                                channel.guild.members.get(guild.uid[user]).displayName
                            })이 ${problem}번 문제를 풀었습니다!`
                        )
                    ...
                    if (
                        bot.guilds[gid].enabled.includes('레벨') &&
                        백준.guild[gid].hasOwnProperty('quest')
                    ) {
                                ...
                                if (!백준.guild[gid].quest.first_cleared.includes(user)) {
                                    const embed = new RichEmbed()
                                        ...
                                        .addField(
                                            '오늘의 첫 퀘스트 완료!',
                                            `${user}가 경험치를 ${
                                                quest.table[solvedData.level]
                                            }+3000 얻었습니다.`
                                        )
                                    레벨.earnXP(
                                        gid,
                                        guild.uid[user],
                                        quest.table[solvedData.level] + 3000
                                    )
