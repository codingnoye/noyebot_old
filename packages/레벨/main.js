...
const package = {
    ...
    help(msg) {
        ...
        for (const keyword of commands.keywords) {
            ...
            if (help.args)
                embed.addField(
                    `\`${prefix}${keyword} ${help.args
                        .split(' ')
                        .map((x) => '<' + x + '>')
                        .join(' ')}\``,
                    help.desc
                )
            else embed.addField(`\`${prefix}${keyword}\``, help.desc)
        }
    ...
    helpSimple(msg, embed) {
        ...
        for (const keyword of commands.keywords) {
            ...
            else helps.push(`\`${prefix}${keyword}\`\n${help.desc}`)
        }
    ...
    earnXP(gid, uid, ammount) {
        if (this.guilds[gid].xp.hasOwnProperty(uid)) this.guilds[gid].xp[uid] += ammount
        else this.guilds[gid].xp[uid] = ammount
        this.levelUpCheck(gid, uid)
        store.save(`레벨/${gid}`)
    },

    levelUpCheck(gid, uid) {
        if (!this.guilds[gid].lv.hasOwnProperty(uid)) this.guilds[gid].lv[uid] = 1
        ...
        if (this.guilds[gid].lv[uid] < newLv) {
            ...
            const embed = new RichEmbed()
                ...
                .setTitle(`레벨 업!`)
                .addField(
                    `**Lv. ${this.guilds[gid].lv[uid]}** -> **Lv. ${newLv}** (${Math.round(
                        ((rawXP - level.xpTable[newLv - 1]) /
                            (level.xpTable[newLv] - level.xpTable[newLv - 1])) *
                            100
                    )}%)`,
                    `**${channel.guild.members.get(uid).displayName}**\n${comma(
                        rawXP - level.xpTable[newLv - 1]
                    )}xp / ${comma(
                        level.xpTable[newLv] - level.xpTable[newLv - 1]
